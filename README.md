...
Commands:
  qiskitjs backend [name]                   Get latest calibration stats for a
                                            backend                 [aliases: b]
  qiskitjs backends [onlySims]              Get latest calibration stats for a
                                            backend                [aliases: bs]
  qiskitjs calibration [backend]            Get latest calibration stats for a
                                            backend                 [aliases: c]
  qiskitjs circuit <circuit> [backend]      Send the circuit to be run in the
  [shots] [name] [seed] [maxCredits]        Quantum Experience
                                            (https://quantumexperience.ng.bluemi
                                            x.net)                 [aliases: ci]
  qiskitjs credits                          Information about your credits
                                                                   [aliases: cr]
  qiskitjs factor <number>                  Calculate a factor of a number
                                                                    [aliases: f]
  qiskitjs job <id>                         Get the info of a specific job
                                                                    [aliases: j]
  qiskitjs jobs [limit] [offset]            Get all your jobs. Ordered by
                                            creation date          [aliases: js]
  qiskitjs login [printToken]               Use an API key to get a token to
                                            make requests           [aliases: l]
  qiskitjs params [backend]                 Get latest parameter stats for a
                                            backend                 [aliases: p]
  qiskitjs parse <circuit>                  Parse the circuit to our IR
                                            (intermediate representation - JSON
                                            format).                [aliases: p]
  qiskitjs queue [backend]                  Get the status of a backend queue
                                                                    [aliases: q]
  qiskitjs random [engine] [length]         Generate a true random number among
  [format] [backend]                        0 and 1                 [aliases: r]
  qiskitjs result <jobId>                   Get the result of a background job
                                            generated by an algorithm method.
                                            The engine is ommited because IBM Q
                                            is the unique which uses background
                                            jobs for now.          [aliases: re]
